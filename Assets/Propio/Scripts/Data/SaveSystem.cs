...
{
    #region Player
    public static void SavePlayer(GameObject player, int score, GameObject camera)
    {
        ...
        PlayerData data = new PlayerData(player, score, camera);
    ...
    #region Ranking
    public static void SaveRanking(string[] name, int[] score, int[] date, int sizeRanking)
    {
        string path = Application.persistentDataPath + "/ranking.data";

        FileStream stream = new FileStream(path, FileMode.Create);

        RankingData data = new RankingData(name, score, date, sizeRanking);
